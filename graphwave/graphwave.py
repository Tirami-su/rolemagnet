...
def heat_diffusion_ind(graph, taus=TAUS, order = ORDER, proc = PROC):
    ...
    if proc == 'exact':
        ...
        heat = {i: sc.sparse.csc_matrix((n_nodes, n_nodes)) for i in range(n_filters) }# matrix 0
...
def graphwave_alg(graph, time_pnts, node='', taus= 'auto', 
    ...
    verbose           :   the algorithm prints some of the hidden parameters
                          as it goes along
    approximate_lambda:   (boolean) should the range of lambda be approximated or
                          computed?
    proc              :   which procedure to compute the signatures (approximate == that
                          is, with Chebychev approx -- or exact)
    ...
    chi = charac_function_multiscale(heat_print, node, time_pnts)
